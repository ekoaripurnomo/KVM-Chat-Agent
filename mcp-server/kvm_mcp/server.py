...
from .vm.images import download_master_image
...
async def handle_request(request_str: str) -> str:
    ...
    try:
        ...
        if request["method"] == "initialize":
            ...
            elif tool_name == "download_master_image":
                result = await download_master_image(**arguments)
            elif tool_name == "get_vm_access":
                result = await get_vm_access(**arguments)
            elif tool_name == "delete_vm":
                result = await delete_vm(**arguments)
            else:
